const silverafternoon = require('../db/models/silver.afternoon');
...
exports.adminmoviepost = async(req,res)=>{
  try {
    ...
    let result =  await silverafternoon.query().update(screendata)
  ...
  try{
    ...
    const fieldsToInsert = final.map(field => 
            ({ movie_name:moviename,seat_number:field,seat_availability:'available'})); 
              let result= await silverafternoon.query().insert(fieldsToInsert);
              res.json(result)
...
exports.userticket = async(req,res)=>{
  try {
    ...
    jwt.verify(token, config.secret, async(err, decoded) => {
      ...
      const result = await user.query().withGraphFetched('Screen_Silver_Afternoon').findById(value)
       .then(specific=>{
         res.json({
          name:specific.name,
          movie_name:specific.Screen_Silver_Afternoon[0].movie_name,
          seat_number:specific.Screen_Silver_Afternoon[0].seat_number,
          show_time:'Afternoon',
          screen_name:"Silver"
         })
       })
...
exports.bookticket = async (req,res,next)=> {
  try {
    ...
    const {seat_number} = req.query
           await silverafternoon.query().findOne('seat_number',seat_number).then(async hello =>{ 
      if(hello.seat_availability == "available"){
        const result = await silverafternoon.query().where({seat_number:req.query.seat_number}).update(screendata)
...
exports.deletespecificticket = async(req,res)=>{
  try {
    ...
    jwt.verify(token, config.secret, async(err, decoded) => {
      ...
      const {seat_number} = req.query
      await silverafternoon.query().findOne('seat_number',seat_number).then(async value =>{
        if(value.Gold_users == userId){
          const result = await silverafternoon.query().where({seat_number:req.query.seat_number}).update(seatdata);
        ...
        return res.status(401).send({
          message: "Unauthorized!"
           ...
           const userId = decoded.id
            ...
            await silverafternoon.query().where({ Gold_users: userId}).patch({seat_availability:"available",Gold_users:0},['Gold_users','seat_availability','Gold_users'])
...
exports.showdetails = async (req,res)=> { 
    try {  
       ...
       let result = await silverafternoon.query().column('movie_name','seat_number','seat_availability')
  ...
  exports.superadmindelete = async(req,res)=>{
    try {
        ...
        if(movie_name == null){
           silverafternoon.query().update(movie_name)
